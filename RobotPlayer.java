...
public class RobotPlayer {
   ...
   */
  private static final int SIG_TRAINING_FIELD = 37;
  ...
  private static Direction compositeEnemyDirection(RobotInfo[] enemies) {
    ...
    for(MapLocation tower : enemyTowers) {
      if(myLocation.distanceSquaredTo(tower) < TOWER.sensorRadiusSquared + 2) {
        loc = loc.add(myLocation.directionTo(tower));
      }
    }

    if(enemies.length == 0) return myLocation.directionTo(loc);
  ...
  private static boolean unsafeDiffuseSafely(RobotInfo[] enemies, Direction preferred) {
    ...
    if(ced == Direction.NONE | ced == Direction.OMNI) ced = (preferred == null? directions[rand.nextInt(8)] : preferred);
    ...
    try {
      ...
      if(destination != null) {
        ...
        // Otherwise, we haven't reached the destination yet, so route to it.
          ...
          Direction[] ds = getDirectionsTo(destination);
