...
public class RobotPlayer {
  ...
  public static void run(RobotController controller) {
    ...
    while(true) {
      ...
      if(Clock.getRoundNum() % 50 == 0 && t != MISSILE) {
      ...
      if(t == HQ) {
        ...
        if(round == 1500) {
          ...
          if(rc.getTeamOre() > 1000) {
            ...
            safeBroadcast(SIG_DRONE_RATE, 15);
            safeBroadcast(SIG_TANK_RATE, 15);
          }
          else {
        ...
        if(round == 1000) {
          ...
          safeBroadcast(SIG_LAUNCHER_RATE, 20);
          if(rc.getTeamOre() > 1000) {
            safeBroadcast(SIG_DRONE_RATE, 5);
            safeBroadcast(SIG_TANK_FACTORY_RATE, 1);
            safeBroadcast(SIG_TANK_RATE, 5);
            safeBroadcast(SIG_AERO_LAB_RATE, 2);
            safeBroadcast(SIG_HANDWASH_RATE, 4);
          }
        }
        if(round == 804) {
          rc.setIndicatorString(0, "Phase 8");
          ...
          if(rc.getTeamOre() > 1000) {
            safeBroadcast(SIG_DRONE_RATE, 5);
            safeBroadcast(SIG_BEAVER_RATE, 7);
            safeBroadcast(SIG_COMMANDER_RATE, 1);
            safeBroadcast(SIG_DEPOT_RATE, 3);
          }
        }
        if(round == 644) {
          ...
          if(rc.getTeamOre() > 1000) {
            safeBroadcast(SIG_DRONE_RATE, 5);
            safeBroadcast(SIG_TECH_INSTITUTE_RATE, 1);
            safeBroadcast(SIG_TRAINING_FIELD_RATE, 1);
            safeBroadcast(SIG_COMMANDER_RATE, 1);
          }
        }
        else if(round == 493) {
          ...
          if(rc.getTeamOre() > 1000) {
            safeBroadcast(SIG_BEAVER_RATE, 6);
            safeBroadcast(SIG_DRONE_RATE, 15);
          }
        }
        else if(round == 393) {
